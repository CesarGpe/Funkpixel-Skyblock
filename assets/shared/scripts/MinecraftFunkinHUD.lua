...
items = { -- you can add custom items here so yeah. {'item texture(in images/items)', 'Display Name', Is it Enchnated? (true/false)}
  {'iron_pickaxe', 'Iron Pickaxe', false}, -- 1
  {'empty', '', false}, -- 2
  {'empty', '', false}, -- 3
  {'empty', '', false}, -- 4
  {'empty', '', false}, -- 5
  {'empty', '', false}, -- 6
  {'stick', 'Debug Mode', true}, -- 7, Debug Menu
  {'armor_stand', 'Character Editor', true}, -- 8, Character Editor
  {'empty', '', true} -- 9
}
...
function onCreatePost()
  ...
  precacheMusic('gameOver-pixel')
  precacheMusic('gameOverEnd-pixel')
  ...
  makeLuaText('shitscore', 'Score: 0 | Misses: 0 | Accuracy: ??.??% [?]', screenWidth, 0, baseCoords[2] + (downscroll and 130 or -80))
  ...
  makeLuaText('itemname', '', screenWidth, 0, (downscroll and 220 or 550))
...
function onUpdate(elapsed)
  local keys = {'ONE', '', '', '', '', '', 'SEVEN', 'EIGHT', ''} -- i hate how this works
  for i = 1,9 do 
...
function onUpdatePost(elapsed)
  ...
  daddycolor = rgbToHex(getProperty('boyfriend.healthColorArray'))
...
function noteMiss(membersIndex, noteData, noteType, isSustainNote)
  ...
  doTweenAngle('oof', 'camGame', 3, 0.05, 'linear')
  end 
  if flashingLights then 
  setProperty('boyfriend.color', getColorFromHex('FF4D4D'))
  end
  runTimer('oofie', 0.1)
end
