...
function onPause()
    if getPropertyFromClass('PlayState', 'chartingMode') then -- oops
...
function onCustomSubstateCreate(name)
    if name == 'minecraftPause' then
        ...
        makeButton('advance', 'Advancements', 350, 230, false, 'other', 0.5)
        makeButton('restart', 'Restart', 650, 230, false, 'other', 0.5)
...
function onCustomSubstateUpdate(name, elapsed)
    ...
    if name == 'udied' then
		...
		if keyboardJustPressed('ENTER') then
			onMCButtonClick('button-restart')
		end
		if keyboardJustPressed('SPACE') then
			onMCButtonClick('button-restart')
		end
		if keyboardJustPressed('ESCAPE') then
			onMCButtonClick('button-quit')
		end
    end
    if name == 'optionsmenu' then
        buttonShit()
